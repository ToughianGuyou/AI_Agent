...
- 这是事实追溯的源头。
- 用户剪藏与已有 `Clippings/` 文件默认只读；Agent 不得修改、覆盖、移动、重命名或删除它们，除非用户对该操作另行明确确认。
- 仅当用户明确授权搜集、下载、剪藏或保存来源时，Agent 才可受控地在 `Clippings/` 新增文件和目录；不得将此授权扩大为对既有资料的编辑权限。
- 新增前必须比较目标相对路径与 SHA-256：同名同哈希时复用既有文件；同名异哈希时停止写入并报告冲突，不得覆盖或自行改名。
- 下载内容必须先写入临时文件，完成来源与完整性校验后再移入目标路径；校验失败时不得留下目标文件。
- 每份 Agent 新增资料必须记录可追溯的来源 URL、抓取时间与 SHA-256，并在新增后进入增量 Ingest。
...
除用户明确要求外，Agent 的自动写入范围仅限 `topics/` 和 `system/`。用户明确授权的来源搜集任务可按 §2.1 受控新增 `Clippings/` 内容；修改根目录规则文件属于配置变更，必须由用户明确发起。
...
6. 用户明确授权搜集任务时，按 §2.1 完成受控新增，并记录每份新增资料的来源 URL、抓取时间和 SHA-256。
7. 如果存在新增、变更或失败待重试的资料，在取得写入锁后先执行增量 Ingest。
8. 如果没有待处理资料，直接继续当前任务，不制造空提交或无意义日志。
...
Ingest 必须可重复运行且不产生重复知识；通过 §2.1 受控新增的每份资料均必须进入本流程：
...
- 修改、覆盖、移动、重命名或删除既有 `Clippings/` 内容；用户明确授权的受控新增除外，且仍须遵守 §2.1 的去重、冲突和下载校验规则。
- 删除 Wiki 页面。
- 大范围重命名或合并主题。
- 清除无法确认归属的写入锁。
- 覆盖与当前任务重叠的未提交修改。
